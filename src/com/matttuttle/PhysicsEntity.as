	...
	public class PhysicsEntity extends Entity
	{
		...
		private function applyVelocity():void
		{
			...
			for (i = 0; i < Math.abs(velocity.x); i++)
			{
				...
				{
					//Here to check for boundries..
					var newX:int = x + FP.sign(velocity.x);
					if(newX > 0 && newX < GameState(this.world).getLevel().getWidth() * Level.tileWidth - this.width) x += FP.sign(velocity.x);
				}
			}
